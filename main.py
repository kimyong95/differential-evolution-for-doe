...
def obj_func(x: tuple, *args):
    ...
    z = np.ones((support_points,1))
    r = np.append(z,x, 1) # p x (1 + factor)
    ...
    s = np.asanyarray(s)
    information_matrix = np.sum(s, axis=0)
...
bounds = np.full((support_points, 1+design_factor,2 ), 0, dtype="float")
bounds[:,:,:] = np.array((-1,1)) # design factor bounds

bounds[:,0,:] = np.array((0,2/support_points)) # replicates probability bounds
